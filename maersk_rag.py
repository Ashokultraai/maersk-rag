...
def db_exists():
    return (
        ...
        and any(f.endswith(".faiss") for f in os.listdir(PERSIST_DIR))
    )
...
# ─────────────────────────────────────────────
# RAG CHAIN (modern LCEL — no RetrievalQA)
# ─────────────────────────────────────────────

def build_rag_chain(vectorstore):
    prompt = PromptTemplate.from_template("""You are a global shipping industry analyst. \
Use the context below to answer the question as specifically and factually as possible. \
Include numbers, statistics, company names, and dates when available. \
Do NOT say "I don't have information" if relevant data exists in the context.
...
with st.sidebar:
    st.image(
        "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e2/Maersk_Logo.svg/320px-Maersk_Logo.svg.png",
        width=140
    )
    st.markdown("---")
    ...
    if db_exists() and last_updated:
        ...
        label = "✅ Fresh" if not is_db_stale() else "⚠️ Stale"
        css = "db-status-ok" if not is_db_stale() else "db-status-old"
...
def run_build(force_rebuild=False):
    ...
    if force_rebuild and os.path.exists(PERSIST_DIR):
        ...
        try:
            ...
            st.error(f"❌ Could not delete old DB: {e}\n\nTry manually deleting the `shipping_db` folder.")
            return
...
if db_exists() and st.session_state.rag_chain is None and st.session_state.api_key and not st.session_state.db_loaded:
    try:
        vs = load_vectorstore()
        chain, retriever = build_rag_chain(vs)
        st.session_state.rag_chain = chain
...
# ── Main Chat Area ────────────────────────────────────────────────────────────
if not st.session_state.api_key:
    st.error("⚠️ OpenAI API key not found. Please add `OPENAI_API_KEY=sk-...` to your `.env` file and restart.")
elif not db_exists():
    ...
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"], avatar="🧑" if msg["role"] == "user" else "🚢"):
            ...
            if msg["role"] == "assistant" and "sources" in msg:
                st.markdown(
                    " ".join([f'<span class="source-tag">📌 {s}</span>' for s in msg["sources"]]),
                    unsafe_allow_html=True
                )

    # Get question from input or button
    prompt = None
        ...
        with st.chat_message("assistant", avatar="🚢"):
            if st.session_state.rag_chain is None:
                ...
                with st.spinner("Searching knowledge base..."):
                    ...
                    sources = list({doc.metadata["label"] for doc in source_docs})
